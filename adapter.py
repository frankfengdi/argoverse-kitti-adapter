...
cam_id = 0 # Choose only one of camera as reference
cam = cams_all[cam_id]
...
if not os.path.exists(goal_dir+'/ImageSets'): os.mkdir(goal_dir+'/ImageSets') # to store train val mapping
     ...
     """.format(L1,L2,L3,L4,L5,L6,L7)
        ...
        for frame_idx in frame_idx_list[::sample_rate]:
            ...
            if dr == 'val/' and has_object: 
                file=open(goal_subdir+'../ImageSets/val.txt','a')
                file.write(str(kitti_idx).zfill(6)+' \n')
                file.close()
            elif dr == 'test/' and has_object:
                file=open(goal_subdir+'../ImageSets/test.txt','a')
                file.write(str(kitti_idx).zfill(6)+' \n')
                file.close()
            elif has_object: # training
                file=open(goal_subdir+'../ImageSets/train.txt','a')
                file.write(str(kitti_idx).zfill(6)+' \n')
                file.close()
